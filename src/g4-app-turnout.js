...
function restyleMapForTurnout() {
  ...
  /* The census layer shades by aggregate turnout too, and its value is
     computed from state.turnout.weight like the other two. Leaving it out
     meant moving the weight restyled the federal and provincial layers and
     left the census one showing the previous weighting. */
...
/* The aggregate weight is read in three places -- the map's turnout-agg
   ...
   aggregate column -- so the socio side has to be recomputed, not merely
   repainted. */
for (const id of ['turnout-unit', 'turnout-weight', 'turnout-min-electors']) {
  $(id).addEventListener('change', () => { refreshTurnout(); refreshSocio(); });
}
/* Apportionment is different from the other three: it changes WHICH BALLOTS
   every reader sees, not just how the turnout table presents them. The map,
   the readout and the correlation are all computed from the apportioned
   values, so all of them have to follow.
   ...
   numbers that move. */
for (const id of ['apportion-fed', 'apportion-prov']) {
