...
/* The chosen measures, described as they are NOW rather than as they were when
   they were chosen.

   Adding a measure stored the whole catalogue entry, label and all, and a label
   is built from things that move underneath it. The roll's noun is the one that
   bit. "Call them" was set to electors after the door measure was already on
   the list, so one export carried a count column headed `electors` -- read live
   at export time -- beside measure_3_name reading "How many addresses are at
   the address", frozen at the moment it was added. Same file, same numbers, two
   nouns, and the stale one was in the column a client reads for the WHY.

   The same staleness reaches every label built from loaded data: a census
   variable that gets its proper name when a better profile is loaded, a
   non-voter pairing, a party list rebuilt from a new results file. So what is
   stored is only what a reader DECIDED -- which measure, which direction, what
   weight -- and everything that describes it is looked up again by id.

   A measure the catalogue no longer offers keeps what it was stored with. It
   has stopped resolving, which the basis line warns about and the export
   refuses outright, and a stale name is the least of what is wrong with it. */
function liveTargets() {
  const targets = state.targets || [];
  if (!targets.length) return [];
  const now = new Map(measureCatalogue().map((m) => [m.id, m]));
  return targets.map((m) => {
    const live = now.get(m.id);
    return live ? { ...live, invert: m.invert, weight: m.weight } : m;
  });
}
...
function exportBases() {
  const chosen = liveTargets().map(basisFor).filter(Boolean);
...
function refreshTargetPicker() {
  ...
  if (list) {
    ...
    /* What each measure is actually worth, which is the question a reader asks
       ...
       spelled. */
    const chosen = liveTargets();
    const total = chosen.reduce((a, x) => a + weightOf(x), 0);
    chosen.forEach((m, i) => {
      const li = document.createElement('li');
...
if ($('file-points')) {
  ...
  $('points-noun').addEventListener('change', () => {
    if (state.points) {
      ...
      /* The noun names a measure as well as a column, so the chosen list and
         the line under it have to be redrawn or the screen goes on showing the
         old word while the file that leaves the tab carries the new one --
         which is the disagreement this whole fix exists to remove. */
      refreshTargetPicker(); renderExportBasis();
    }
  });
